...
use ghost::{Policy, get_aws_info, AwsFunctionInfo, Event};
...
hook!{
    unsafe fn getaddrinfo(node: *const char, service: *const char, hints: *const libc::addrinfo, res: *mut *mut libc::addrinfo) -> c_int => t_getaddrinfo{
        //println!("IN GETADDR");
        log::debug!("[IRON FUNCTION ACTION]: In GETADDR");
...
fn allow_filesystem_access(path: &str) -> bool {
    ...
    if POLICY.read_write_tmp == "block"{
        ...
        let full_path = match fs::canonicalize(path){
            Ok(full_path) => String::from(full_path.as_os_str().to_str().unwrap()),
            _ => String::from(" ")            
        };

        log::debug!("IN ALLOW FILESYSTEM ACCESS {}", full_path);
      

        if path == "/tmp" || full_path == "/tmp" {
            return false
        }
    }
...
fn parse_policy() -> Policy{
    ...
    let policy_path = env::var("IRON_FUNCTION_POLICY").expect("IRON FUNCTION [ERROR] IRON_FUNCTION_POLICY env not found");
...
fn create_log(action: &str, msg: &str){
    log::debug!("[IRON FUNCTION ACTION]: {} -> {}", action, msg);
}
